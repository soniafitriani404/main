...
-- VIEW 1: Gabungan dari minimal 2 tabel
CREATE OR REPLACE VIEW view_pelanggan_pemesanan AS
SELECT DISTINCT
    ...
    Pemesanan.status
FROM Pelanggan
JOIN Pemesanan ON Pelanggan.pelanggan_id = Pemesanan.pelanggan_id;

SELECT * FROM view_pelanggan_pemesanan;

-- VIEW 2: Gabungan dari minimal 3 tabel
...
-- VIEW 3: Gabungan tabel dengan kondisi tertentu (status = 'Selesai')
CREATE VIEW view_pemesanan_selesai AS
SELECT 
    Pelanggan.nama AS nama_pelanggan,
    Paket.nama_paket,
    Pemesanan.tanggal_pesan,
...
SELECT * FROM view_pemesanan_selesai;

-- VIEW 4: Agregasi dari 2 tabel
CREATE VIEW view_total_pendapatan_metode AS
SELECT 
    metode_pembayaran,
...
SELECT * FROM view_total_pendapatan_metode;

-- VIEW 5: View berguna – jumlah pesanan per fotografer
CREATE VIEW view_jumlah_pesanan_fotografer AS
SELECT 
    Fotografer.nama AS nama_fotografer,
    COUNT(Pemesanan.pemesanan_id) AS jumlah_pesanan
FROM Fotografer
LEFT JOIN Pemesanan ON Fotografer.fotografer_id = Pemesanan.fotografer_id
GROUP BY Fotografer.fotografer_id;

SELECT * FROM view_jumlah_pesanan_fotografer;
