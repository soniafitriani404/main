...
-- Tabel untuk Skala UMKM
CREATE TABLE skala_umkm (
    id INT AUTO_INCREMENT PRIMARY KEY,
    nama_skala VARCHAR(50),
    batas_aset_bawah INT,
    batas_aset_atas INT,
    batas_omzet_bawah INT,
    batas_omzet_atas INT
);

-- Tabel untuk Kabupaten/Kota di Jawa Barat
CREATE TABLE kabupaten_kota (
    id INT AUTO_INCREMENT PRIMARY KEY,
    nama_kabupaten_kota VARCHAR(100)
);

-- Tabel untuk Pemilik UMKM
...
CREATE TABLE umkm (
    ...
    FOREIGN KEY (id_pemilik) REFERENCES pemilik_umkm(id),
    FOREIGN KEY (id_kategori) REFERENCES kategori_umkm(id),
    FOREIGN KEY (id_skala) REFERENCES skala_umkm(id),
    FOREIGN KEY (id_kabupaten_kota) REFERENCES kabupaten_kota(id)
);

-- Tabel untuk Produk UMKM
...
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id
WHERE u.id_skala = 3; -- Skala Menengah

-- 5. view_umkm_per_kota
CREATE VIEW view_umkm_per_kota AS
SELECT 
    kk.nama_kabupaten_kota AS nama_kota,
