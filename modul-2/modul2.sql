...
-- Perbaikan: Tambahkan koma setelah AUTO_INCREMENT
...
-- Tabel lainnya tetap sama seperti yang Anda miliki
...
-- soal 1.1 (Penjualan dan Pelanggan
...
SELECT 
    ...
    pl.alamat,
    p.tanggal,
    (SELECT SUM(subtotal) FROM DetailPenjualan WHERE id_penjualan = p.id_penjualan) AS total_penjualan
FROM 
    Penjualan p
JOIN 
    Pelanggan pl ON p.id_pelanggan = pl.id_pelanggan;
    
-- soal 1.2 (Penjualan, Pegawai, dan Pelanggan)
...
SELECT 
    ...
    pg.posisi,
    (SELECT SUM(subtotal) FROM DetailPenjualan WHERE id_penjualan = p.id_penjualan) AS total_transaksi
FROM 
    Penjualan p
JOIN 
    Pelanggan pl ON p.id_pelanggan = pl.id_pelanggan
...
-- soal 1.3 Barang dengan stok kurang dari 5

CREATE VIEW view_barang_stok_rendah AS
SELECT 
    b.id_barang,
    b.nama_barang,
    b.harga,
...
-- soal 1.4  (Statistik penjualan per kategori)

CREATE VIEW view_statistik_kategori AS
SELECT 
    k.nama_kategori,
...
GROUP BY 
 ...
 -- soal 1.5  Analisis Produk terlaris
...
SELECT 
    ...
    SUM(dp.jumlah) AS total_terjual,
    SUM(dp.subtotal) AS total_pendapatan,
    RANK() OVER (ORDER BY SUM(dp.jumlah) DESC) AS peringkat_penjualan
FROM 
    DetailPenjualan dp
JOIN 
    Barang b ON dp.id_barang = b.id_barang
JOIN 
    Kategori k ON b.id_kategori = k.id_kategori
GROUP BY 
    b.id_barang, b.nama_barang, k.nama_kategori
ORDER BY 
    total_terjual DESC;
    
    
SELECT * FROM view_penjualan_pelanggan;
SELECT * FROM view_produk_terlaris WHERE peringkat_penjualan <= 5;
