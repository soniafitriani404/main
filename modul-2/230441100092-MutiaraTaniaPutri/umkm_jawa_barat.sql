...
MySQL - 10.4.32-MariaDB : Database - umkm_jawa_barat
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`umkm_jawa_barat` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_produk_umkm` AS select `u`.`nama_usaha` AS `nama_usaha`,`pr`.`nama_produk` AS `nama_produk`,`pr`.`deskripsi_produk` AS `deskripsi_produk`,`pr`.`harga` AS `harga` from (`produk_umkm` `pr` join `umkm` `u` on(`pr`.`id_umkm` = `u`.`id_umkm`)) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_umkm_detail` AS select `u`.`nama_usaha` AS `nama_usaha`,`p`.`nama_lengkap` AS `nama_pemilik`,`k`.`nama_kategori` AS `nama_kategori`,`s`.`nama_skala` AS `nama_skala`,`kota`.`nama_kabupaten_kota` AS `nama_kabupaten_kota`,`u`.`tahun_berdiri` AS `tahun_berdiri` from ((((`umkm` `u` join `pemilik_umkm` `p` on(`u`.`id_pemilik` = `p`.`id_pemilik`)) join `kategori_umkm` `k` on(`u`.`id_kategori` = `k`.`id_kategori`)) join `skala_umkm` `s` on(`u`.`id_skala` = `s`.`id_skala`)) join `kabupaten_kota` `kota` on(`u`.`id_kabupaten_kota` = `kota`.`id_kabupaten_kota`)) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_umkm_per_kota` AS select `kota`.`nama_kabupaten_kota` AS `nama_kabupaten_kota`,count(`u`.`id_umkm`) AS `jumlah_umkm` from (`umkm` `u` join `kabupaten_kota` `kota` on(`u`.`id_kabupaten_kota` = `kota`.`id_kabupaten_kota`)) group by `kota`.`nama_kabupaten_kota` */;
