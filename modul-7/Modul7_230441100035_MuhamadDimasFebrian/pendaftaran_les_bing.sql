...
ALTER TABLE pembayaran ADD COLUMN jumlah_tagihan DECIMAL(10,2);
UPDATE pembayaran SET jumlah_tagihan = 750000 WHERE id_pembayaran = 1;
UPDATE pembayaran SET jumlah_tagihan = 850000 WHERE id_pembayaran = 2;
UPDATE pembayaran SET jumlah_tagihan = 1000000 WHERE id_pembayaran = 3;

ALTER TABLE pendaftaran MODIFY STATUS ENUM('Aktif', 'Tidak Aktif', 'Pasif');
ALTER TABLE pendaftaran ADD COLUMN status_pembayaran VARCHAR(20);
...
-- no1
DELIMITER //
CREATE PROCEDURE tampil_pendaftaran_1_bulan()
BEGIN
    SELECT * FROM pendaftaran
    WHERE tanggal_daftar <= CURDATE() - INTERVAL 1 MONTH;
END //
DELIMITER ;
DROP PROCEDURE IF EXISTS tampil_pendaftaran_berdasarkan_lama;
CALL tampil_pendaftaran_1_bulan();
...
BEGIN
    ...
    WHERE tanggal_bayar < CURDATE() - INTERVAL 1 YEAR
    AND jumlah_bayar > 0;
END //
DELIMITER ;
...
-- no3
DELIMITER //
CREATE PROCEDURE ubah_status_pembayaran()
BEGIN
    UPDATE pembayaran
...
BEGIN
    UPDATE siswa
    SET nama_siswa = p_nama,
        alamat = p_alamat,
        no_telepon = p_nohp,
        email = p_email
    WHERE id_siswa = p_id_siswa
      AND p_id_siswa NOT IN (SELECT id_siswa FROM pendaftaran);
END //
DELIMITER ;
DROP PROCEDURE IF EXISTS edit_user;
CALL edit_user(10, 'Fernanda Gusnan', 'Jl. Baru No.5, Madiun', '082212345678', 'fernanda_update@gmail.com');

 -- no5 Update Status Transaksi Berdasarkan Jumlah dan ID Transaksi 1 Bulan Terakhir
DELIMITER //
...
BEGIN
    ...
    -- Ambil ID dengan jumlah minimum
    SELECT id_pembayaran INTO id_min
    FROM pembayaran
    WHERE tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
    ORDER BY jumlah_bayar ASC
    LIMIT 1;

    -- Ambil ID dengan jumlah maksimum
    SELECT id_pembayaran INTO id_max
    FROM pembayaran
    WHERE tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
    ORDER BY jumlah_bayar DESC
    LIMIT 1;

    -- Ambil ID tengah (status pasif)
    SELECT id_pembayaran INTO id_mid
    FROM (
        SELECT id_pembayaran, jumlah_bayar
        FROM pembayaran
        WHERE tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
        ORDER BY jumlah_bayar
        LIMIT 1 OFFSET 1
    ) AS tengah;
    ...
    IF id_min IS NOT NULL THEN
        ...
        WHERE id_pendaftaran = (SELECT id_pendaftaran FROM pembayaran WHERE id_pembayaran = id_min);
    END IF;

    IF id_mid IS NOT NULL THEN
        UPDATE pendaftaran
        SET STATUS = 'Pasif'
        WHERE id_pendaftaran = (SELECT id_pendaftaran FROM pembayaran WHERE id_pembayaran = id_mid);
    END IF;

    IF id_max IS NOT NULL THEN
        UPDATE pendaftaran
        SET STATUS = 'Aktif'
        WHERE id_pendaftaran = (SELECT id_pendaftaran FROM pembayaran WHERE id_pembayaran = id_max);
    END IF;
...
-- modul 7
-- before Cegah update jika total_bayar < 0
DELIMITER //
CREATE TRIGGER before_update_pembayaran
BEFORE UPDATE ON pembayaran
FOR EACH ROW
BEGIN
  IF NEW.jumlah_bayar < 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Jumlah bayar tidak boleh kurang dari 0';
  END IF;
END;
//
DELIMITER ;
UPDATE pembayaran SET jumlah_bayar = -50000 WHERE id_pembayaran = 1;


-- delete Cegah penghapusan jika pembayaran sudah lunas
DELIMITER //
CREATE TRIGGER before_delete_pembayaran
BEFORE DELETE ON pembayaran
FOR EACH ROW
BEGIN
  IF OLD.jumlah_bayar >= 500000 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Data pembayaran dengan nominal besar tidak boleh dihapus';
  END IF;
END;
//
DELIMITER ;
DELETE FROM pembayaran WHERE jumlah_bayar >= 500000;

-- insert Cek agar total_bayar tidak NULL
DELIMITER //
CREATE TRIGGER before_insert_pembayaran
BEFORE INSERT ON pembayaran
FOR EACH ROW
BEGIN
  IF NEW.jumlah_bayar IS NULL THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Jumlah bayar tidak boleh kosong';
  END IF;
END;
//
DELIMITER ;
INSERT INTO pembayaran (id_pembayaran, id_pendaftaran, tanggal_bayar)
VALUES (100, 1, '2025-05-13');




-- after
-- ubah status jadi "Lunas" jika jumlah_bayar ≥ 500000
DELIMITER //
...
//
DELIMITER ;
UPDATE pembayaran SET jumlah_bayar = 600000 WHERE id_pembayaran = 1;
SELECT id_pendaftaran, status_pembayaran FROM pendaftaran WHERE id_pendaftaran = 1;

-- ubah status jadi "Belum Bayar" jika pembayaran dihapus
DELIMITER //
...
//
DELIMITER ;
SELECT * FROM pendaftaran WHERE id_pendaftaran = 11;
DELETE FROM pembayaran WHERE id_pembayaran = 11;

-- otomatis set status jadi "Belum Lunas"
DELIMITER //
...
//
DELIMITER ;
INSERT INTO pembayaran (id_pembayaran, id_pendaftaran, jumlah_bayar) VALUES (1, 1, 100000);
SELECT * FROM pendaftaran WHERE id_pendaftaran = 1;
UPDATE pembayaran SET jumlah_bayar = 600000 WHERE id_pembayaran = 1;
