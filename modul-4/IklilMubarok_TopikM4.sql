...
SELECT * FROM view_murid_pendaftaran;
SELECT * FROM view_murid_kelas;
SELECT * FROM view_murid_kelas_mahal;
SELECT * FROM view_jumlah_murid_per_kelas;
SELECT * FROM view_detail_kelas;
...
-- contoh pengisian
UPDATE Pendaftaran
SET keterangan = 'Pembayaran lunas'
WHERE id_pendaftaran IN (1, 2, 3);
...
FROM Murid
JOIN Pendaftaran ON Murid.id_murid = Pendaftaran.id_murid;
...
ALTER TABLE Pendaftaran
MODIFY tanggal_pendaftaran DATETIME;

DESC pendaftaran;
SELECT * FROM pendaftaran;

-- 5
-- Menampilkan semua murid, dan kalau mereka pernah daftar, tampilkan juga tanggal daftarnya.
SELECT 
    m.nama AS nama_murid,
    p.tanggal_pendaftaran
FROM Murid m
...
-- Menampilkan semua kelas musik, dan kalau ada pendaftarannya, tampilkan juga ID muridnya.
SELECT 
    k.nama_kelas,
    p.id_murid
FROM Pendaftaran p
RIGHT JOIN KelasMusik k ON p.id_kelas = k.id_kelas;

-- SELF JOIN (contoh: cari guru dengan pengalaman sama)
SELECT 
    g1.nama_guru AS guru_1,
    g2.nama_guru AS guru_2,
    g1.pengalaman_tahun
FROM Guru g1
JOIN Guru g2 ON g1.pengalaman_tahun = g2.pengalaman_tahun
WHERE g1.id_guru < g2.id_guru;


-- 6 Contoh query dengan minimal 5 operator perbandingan:
-- 1. semua murid yang berusia 13 tahun:
SELECT * FROM Murid
WHERE usia = 13;

-- 2. kelas musik yang biayanya lebih dari 200 ribu:
SELECT * FROM KelasMusik
WHERE biaya > 200000;

-- 3. guru yang punya pengalaman kurang dari 5 tahun:
SELECT * FROM Guru
WHERE pengalaman_tahun < 5;

-- 4.  alat musik yang harga sewanya minimal 100 ribu:
SELECT * FROM AlatMusik
WHERE harga_sewa >= 100000;

-- 5. murid yang usia-nya maksimal 14 tahun:
SELECT * FROM Murid
WHERE usia <= 14;

-- 6 guru yang spesialisasinya bukan vokal:
SELECT * FROM Guru
WHERE spesialisasi <> 'Vokal';
