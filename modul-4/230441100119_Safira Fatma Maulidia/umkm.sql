...
#1
CREATE VIEW view_umkm_lengkap AS
SELECT u.id_umkm, u.nama_usaha, p.nama_lengkap AS nama_pemilik,
       k.nama_kategori, s.nama_skala, kk.nama_kabupaten_kota,
       u.tahun_berdiri
FROM umkm u
...
SELECT pr.id_produk, pr.nama_produk, pr.harga, u.nama_usaha
FROM produk_umkm pr
...
SELECT u.nama_usaha, p.nama_lengkap AS nama_pemilik, 
       u.total_aset, u.omzet_per_tahun
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik
JOIN skala_umkm s ON u.id_skala = s.id_skala
...
JOIN umkm u ON kk.id_kabupaten_kota = u.id_kabupaten_kota
GROUP BY kk.nama_kabupaten_kota;

SELECT * FROM view_umkm_lengkap;
...
DROP VIEW IF EXISTS view_total_produk_per_umkm;
...
CALL AddUMKM('Usaha Sejahtera', 15);
SELECT * FROM umkm;
...
CALL getKategori();
CALL UpdateKategoriUMKM(2, 'Kuliner Tradisional');
SELECT * FROM kategori_umkm;
...
-- Contoh pemanggilan:
CALL AddProduk(1, 'Kopi Gula Aren', 25000);
