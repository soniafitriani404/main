...
SELECT u.id_umkm, 
    ...
    u.tahun_berdiri AS Tahun_Berdiri
FROM umkm u
...
SELECT 
    ...
    p.deskripsi_produk AS Deskripsi_Produk,
    p.harga AS Harga
FROM produk_umkm p
...
SELECT 
    u.nama_usaha AS Nama_Usaha,
    p.nama_lengkap,
    u.total_aset AS Total_Aset,
    u.omzet_per_tahun AS Omzet_Per_Tahun
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik
JOIN skala_umkm s ON u.id_skala = s.id_skala
...
BEGIN
    -- Menambahkan data produk ke dalam tabel produk_umkm
...
CALL AddProduk(1, 'Produk A', 50000.00);
...
BEGIN
    -- Mengambil data UMKM berdasarkan id_umkm
...
CREATE PROCEDURE getUmkmJabarByID(IN p_id INT)

BEGIN

SELECT * FROM umkm WHERE id_umkm = p_id;

END//

DELIMITER;

CALL getUmkmJabarByID(4);
...
CREATE PROCEDURE namaDekan(IN p_namaLengkap VARCHAR(200), IN gelar VARCHAR(200))

BEGIN
DECLARE nama VARCHAR (250);
SET nama= CONCAT (p_namaLengkap," ", gelar);
SELECT nama AS namalengkapDekan;

END//

DELIMITER;

CALL namaDekan ("dhelia agustina","S.Kom., M.IT");

-- percabagan barching
DELIMITER//
CREATE PROCEDURE seleksiNilai(IN p_nilai INT)

BEGIN
DECLARE keterangan VARCHAR(200);
IF (p_nilai >= 50) THEN
	SET keterangan = "lu lulus";
ELSE
	SET keterangan = " lu ngulang";
END IF;

SELECT keterangan AS hasil_lu;

END//

DELIMITER;

CALL seleksiNilai(50);

-- looping while
DELIMITER//
CREATE PROCEDURE faktorial (IN bilangan INT) 

BEGIN
DECLARE v_i INT DEFAULT 1;
DECLARE v_faktorial INT default1;

WHILE v_i <= bilangan DO 
	SET v_faktorial = v_faktorial*v_i;
	SET v_i = v_i + 1;

END WHILE//

SELECT v_faktorial AS hasil_faktorial;
