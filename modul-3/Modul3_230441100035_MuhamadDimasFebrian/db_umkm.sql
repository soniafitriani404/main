...
ALTER TABLE kategori_umkm MODIFY COLUMN id_kategori INT NOT NULL AUTO_INCREMENT;
...
ALTER TABLE skala_umkm MODIFY COLUMN id_skala INT NOT NULL AUTO_INCREMENT;
ALTER TABLE skala_umkm ADD COLUMN batas_omzet_bawah DECIMAL(15,2);
...
ALTER TABLE kabupaten_kota MODIFY COLUMN id_kabupaten_kota INT NOT NULL AUTO_INCREMENT;
...
ALTER TABLE pemilik_umkm MODIFY COLUMN id_pemilik INT NOT NULL AUTO_INCREMENT;
...
ALTER TABLE umkm CHANGE COLUMN omzet_per_bulan omzet_per_tahun DECIMAL(15, 2);
UPDATE umkm SET omzet_per_tahun = omzet_per_bulan * 12;
ALTER TABLE umkm MODIFY COLUMN id_umkm INT NOT NULL AUTO_INCREMENT;
...
ALTER TABLE produk_umkm MODIFY COLUMN id_produk INT NOT NULL AUTO_INCREMENT;
...
-- query untuk soal
DROP VIEW IF EXISTS v_umkm_detail;
CREATE VIEW view_umkm_detail AS SELECT 
u.nama_usaha,
p.nama_lengkap AS nama_pemilik,
k.nama_kategori,
s.nama_skala AS skala_usaha,
l.nama_kabupaten_kota,
u.tahun_berdiri
FROM umkm u 
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik 
JOIN kategori_umkm k ON u.id_kategori = k.id_kategori 
JOIN kabupaten_kota l ON u.id_kabupaten_kota = l.id_kabupaten_kota
JOIN skala_umkm s ON u.id_skala = s.id_skala;


CREATE VIEW view_pemilik_dan_usaha AS SELECT
a.nik,
a.nama_lengkap AS nama_pemilik,
a.jenis_kelamin,
a.nomor_telepon,
a.email,
b.nama_usaha
FROM pemilik_umkm a JOIN umkm b ON a.id_pemilik = b.id_pemilik;
...
p.deskripsi_produk,
p.harga
FROM produk_umkm p JOIN umkm u ON p.id_umkm = u.id_umkm;


CREATE VIEW view_umkm_menengah AS SELECT 
u.nama_usaha,
p.nama_lengkap AS nama_pemilik,
u.total_aset,
u.omzet_per_tahun
...
CREATE VIEW view_umkm_per_kota AS SELECT 
...
END //
DELIMITER ;
CALL AddUMKM('Bakso Pak Joko', 5);
CALL AddUMKM('Bakso Pak Wowo', 58);
...
END //
DELIMITER ;
CALL UpdateKategoriUMKM(2, 'Bumbu Kacang Khas Kebumen');
...
END //
DELIMITER ;
DELETE FROM umkm WHERE id_pemilik = 19;
CALL DeletePemilikUMKM(18);
...
END //
DELIMITER ;
CALL AddProduk(18, 'Bumbu Kacang Khas Kebumen', 20000);
SELECT * FROM umkm WHERE id_umkm = 37;

INSERT INTO umkm (id_pemilik, id_kategori, id_skala, id_kabupaten_kota, nama_usaha, jumlah_karyawan)
VALUES (1, 2, 1, 1, 'UMKM Bumbu Kacang', 4);

SELECT * FROM umkm WHERE nama_usaha = 'UMKM Bumbu Kacang';
...
END //
DELIMITER ;
CALL GetUMKMByID(18, @nama_usaha);
SELECT @nama_usaha;
