...
CREATE PROCEDURE tampil_pendaftaran_berdasarkan_lama(IN pilihan VARCHAR(20))
BEGIN
    ...
    ELSE
        SELECT 'Pilihan tidak valid. Gunakan: SEMINGGU, 1 BULAN, atau 3 BULAN' AS pesan;
        LEAVE tampil_pendaftaran_berdasarkan_lama;
    END IF;

    SELECT * FROM pendaftaran
    WHERE tanggal_daftar >= batas_tanggal;
END //
...
BEGIN
    IF EXISTS (
        ...
        SELECT 'User memiliki transaksi dan tidak dapat diedit.' AS pesan;
    ELSE
        UPDATE siswa
        SET nama_siswa = pnama,
            alamat = palamat,
            no_telepon = ptelepon,
            email = pemail
        WHERE id_siswa = pid_siswa;
...
CREATE PROCEDURE update_status_pendaftaran_berdasarkan_jumlah()
BEGIN
    DECLARE id_terendah INT;
    DECLARE id_tertinggi INT;
    DECLARE rata DECIMAL(10,2);

    -- Ambil nilai transaksi dalam 1 bulan terakhir
    SET id_terendah = (
        SELECT p.id_pendaftaran
        FROM pembayaran pb
        JOIN pendaftaran p ON pb.id_pendaftaran = p.id_pendaftaran
        WHERE pb.tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
        ORDER BY pb.jumlah_bayar ASC
        LIMIT 1
    );
    ...
    SET id_tertinggi = (
        ...
        JOIN pendaftaran p ON pb.id_pendaftaran = p.id_pendaftaran
        WHERE pb.tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
        ORDER BY pb.jumlah_bayar DESC
        LIMIT 1
    );

    SET rata = (
        SELECT AVG(pb.jumlah_bayar)
        FROM pembayaran pb
        WHERE pb.tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
    );

    -- Ubah status sesuai ketentuan
    UPDATE pendaftaran
    SET STATUS = 'Tidak Aktif'
    WHERE id_pendaftaran = id_terendah;

    UPDATE pendaftaran
    SET STATUS = 'Aktif'
    WHERE id_pendaftaran = id_tertinggi;

    UPDATE pendaftaran p
    JOIN pembayaran pb ON p.id_pendaftaran = pb.id_pendaftaran
    SET p.status = 'Pasif'
    WHERE pb.jumlah_bayar BETWEEN rata - 50000 AND rata + 50000
    AND pb.tanggal_bayar >= CURDATE() - INTERVAL 1 MONTH
    AND p.id_pendaftaran NOT IN (id_terendah, id_tertinggi);
END //
...
BEGIN
    ...
    DECLARE count_transaksi INT DEFAULT 0;
    ...
    read_loop: LOOP
        ...
        IF done THEN
            LEAVE read_loop;
        END IF;
        SET count_transaksi = count_transaksi + 1;
    END LOOP;
    CLOSE cur;

    SELECT count_transaksi AS jumlah_transaksi_berhasil;
END //
